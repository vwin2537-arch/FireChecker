...
| 📅🔔 เจ้าหน้าที่ลงวันหยุด/ลา **ทุกรายการ** (รวมวันหยุดในโควต้าที่ LINE กลุ่มเงียบ — พี่วินขอ 7 ก.ย. 2026) | แอดมินทุกคน | `h_dayoff_add` (push อย่างเดียว ไม่เขียนกล่องข้อความ · หัวข้อ: 📅 ลงวันหยุด / 🔔 ขอใช้วันหยุดเกินโควต้า / 🔔 มีคำขอลา) |
| ❌ เจ้าหน้าที่ยกเลิกวันหยุด/ลา ทุกรายการ | แอดมินทุกคน | `h_dayoff_cancel` (push อย่างเดียว · LINE กลุ่มยังแจ้งเฉพาะยกเลิกป่วย/กิจ) |
...
- **ไอคอนบน status bar (`badge`) เป็นขาว-ดำเสมอ — Android เอาแค่ alpha channel มาทำ mask** (opaque→ขาว, โปร่งใส→หาย) เดิมตั้ง `badge: 'icon-192.png'` ที่ทึบทั้งใบ → เจ้าหน้าที่เห็นเป็น **สี่เหลี่ยมขาว** บน status bar (เจอจริง 6 ก.ย. 2026) · แก้ด้วย `public/badge-96.png` = silhouette เปลวไฟ (เจาะช่องเช็คถูก) ขาวบนพื้นโปร่งใส 96×96 — สร้างจาก `icon-512.png` ด้วย PIL: alpha=255 ตรง pixel ที่ `r>g+30` (ส้ม=เปลวไฟ) → crop bbox +8% → จัตุรัส → LANCZOS ย่อ 96 (สคริปต์ไม่ได้เก็บไว้ ทำซ้ำได้ใน ~10 บรรทัด) · `icon:` คงเป็น `icon-192.png` (รูปใหญ่ในตัวแจ้งเตือน โชว์สีได้) · badge โหลดตอนแสดงผล → **ต้องใส่ใน `ASSETS` ของ sw.js** ไม่งั้นออฟไลน์ไม่มีไอคอน · อยาก**สีเต็มแบบโลโก้ LINE ทำไม่ได้** เป็นข้อจำกัดของ Android เอง
