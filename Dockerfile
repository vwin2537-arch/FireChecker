...
# รองรับรูป base64 ก้อนใหญ่ + ห้ามพ่น warning ใส่ response (ให้ลง log แทน — กัน JSON พัง)
...
CMD ["bash", "-c", "rm -f /etc/apache2/mods-enabled/mpm_event.* /etc/apache2/mods-enabled/mpm_worker.* && mkdir -p \"${UPLOAD_DIR:-/data/uploads}\" && chown -R www-data:www-data \"${UPLOAD_DIR:-/data/uploads}\" && sed -i \"s/Listen 80/Listen ${PORT:-80}/\" /etc/apache2/ports.conf && sed -i \"s/:80>/:${PORT:-80}>/\" /etc/apache2/sites-available/000-default.conf && apache2-foreground"]
