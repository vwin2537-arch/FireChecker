...
**Verify (Chrome + FCM จริง, login เป็นเจ้าหน้าที่):** กดปิด → ปุ่มเป็น "เปิด" ทันที · reload → ยังเป็น "เปิด" + ไม่ subscribe กลับ + ไม่มี notification หลุด · กดเปิดใหม่ → เด้ง "เปิดแจ้งเตือนสำเร็จ" + ปุ่มเป็น "ปิด" + ธงถูกล้าง

### v38 แก้เคส "กดทดสอบแล้วมือถือไม่เด้ง" (6 ก.ย. 2026) — ✅ deploy แล้ว
พี่วินเทสต์บน prod แล้วมือถือเงียบ — ไล่ log Railway เจอสาเหตุ: **ไม่ใช่บั๊กของ push** แต่มือถือ logout จากแอดมินไป **login เป็นเจ้าหน้าที่** (11:25:44 `app_data` ไม่ใช่ `admin_data`) แล้วกดเปิดแจ้งเตือน (`push_subscribe` 200) ส่วนปุ่ม "ส่งทดสอบ" ที่สำเร็จกดจาก Mac ซึ่ง `h_push_test` ยิงหา **แอดมินเท่านั้น** → **ไม่เคยมี push ถูกยิงไปหามือถือเลย** · และ log ไม่มี `settings_save` เลย = สวิตช์ `push_enabled` ยังไม่ได้เปิด
แก้ 2 อย่าง: (ก) **กดเปิดแจ้งเตือน = เด้งยืนยันกลับมาทันที** (`welcome=1` → server ยิงเข้า subscription ที่เพิ่งบันทึก) เห็นผลกับตาทุกคนไม่ต้องพึ่งหัวหน้าทดสอบให้ (ข) **ปุ่ม "ทดสอบ" รายคนในตารางอุปกรณ์** (`push_test` รับ `user_id`) หัวหน้าเช็คได้ว่า จนท. คนไหนรับได้จริง · เปลี่ยนป้ายปุ่มเป็น "ส่งทดสอบหาตัวเอง" ให้ตรงความจริง
**Verify:** Chrome + FCM จริง — กดเปิด → ได้ "🔔 เปิดแจ้งเตือนสำเร็จ" เด้งจริง · **reload แอปซ้ำ (resync เงียบ) ไม่เด้งเลย** (กับดักสำคัญ: report() เรียก sync() ทุกครั้งที่เปิดแอป) · ยิงทดสอบหา จนท. รายคนถึงเครื่องถูกคน · คนที่ยังไม่เปิดขึ้นข้อความบอกชัด · cache `firecheck-v38` · `FACE_CACHE` ไม่ถูกแตะ
...
**เก็บ device info** ตาราง `user_devices` — หน้าตั้งค่าโชว์เลยว่าใครพร้อมรับ push / ใคร (iPhone) ยังต้องเพิ่มลงหน้าจอโฮมก่อน เพราะพี่วินไม่แน่ใจว่าเจ้าหน้าที่ใช้เครื่องอะไรกันบ้าง
**ส่งขึ้นแบบสวิตช์ปิด** (`push_enabled=0`) — ต้องกดสร้างกุญแจ VAPID ในหน้าตั้งค่าก่อน แล้วค่อยเปิด

**Verify (6 ก.ย. 2026):** เขียนตัวถอดรหัสฝั่งผู้รับด้วย Node แยกอิสระ → ถอดของที่ PHP เข้ารหัสได้ตรงทุกไบต์ (ไทย+emoji ไม่เพี้ยน) + ตรวจลายเซ็น JWT ES256 ผ่าน · **วน 120 รอบด้วยกุญแจสุ่มใหม่ ไม่พังสักรอบ** (จับบั๊กเลข 0 นำหน้า) · รัน push service ปลอมแล้วยิงจริงผ่าน API: `push_test` / ประกาศ broadcast (ออก 3 คนที่ subscribe เท่านั้น) / อนุมัติลา / ขอลา→เด้งหาหัวหน้า / cron เตือน (21 คน − เช็คแล้ว 1 − ลา 1 = 19 เป้าหมาย) / กันส่งซ้ำ / key ผิดถูกปฏิเสธ / **410 = ลบ subscription ตายทิ้งอัตโนมัติ** · สวิตช์ปิด = ไม่มี push ออกเลยแต่กล่องข้อความยังทำงาน · **migration ทดสอบบน DB เดิมที่มีข้อมูลอยู่แล้ว — สร้าง 3 ตารางใหม่ + settings ครบ ไม่แตะของเดิม** · `php -l` + `node --check` ผ่านทุกไฟล์
